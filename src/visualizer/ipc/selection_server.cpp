...
bool SelectionServer::start(const std::string& socket_path) {
    ...
    if (server_fd_ < 0) {
        LOG_ERROR("SelectionServer: socket() failed");
        return false;
    }
    ...
    if (bind(server_fd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        LOG_ERROR("SelectionServer: bind() failed");
        close(server_fd_);
        server_fd_ = -1;
        return false;
    }
    ...
    if (listen(server_fd_, LISTEN_BACKLOG) < 0) {
        LOG_ERROR("SelectionServer: listen() failed");
        close(server_fd_);
        server_fd_ = -1;
        return false;
    }
...
void SelectionServer::handle_client(int client_fd) {
